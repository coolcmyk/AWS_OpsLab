...
data "aws_iam_policy_document" "lambda" {
  statement {
    ...
    resources = ["arn:aws:s3:::${var.evidence_bucket_name}/findings/*"]
  }
  statement {
...
resource "aws_iam_role_policy" "lambda" {
  name   = "${var.name}-enricher-runtime"
  role   = aws_iam_role.lambda.id
  policy = data.aws_iam_policy_document.lambda.json
}
resource "aws_lambda_function" "this" {
  function_name    = "${var.name}-enricher"
  ...
  environment {
    variables = {
      ...
      DB_SECRET_ARN   = var.database_url_secret_arn
    }
  }
  dead_letter_config { target_arn = aws_sqs_queue.dlq.arn }
}
resource "aws_cloudwatch_event_rule" "finding" {
  name           = "${var.name}-finding"
  event_bus_name = aws_cloudwatch_event_bus.this.name
  event_pattern  = jsonencode({ source = ["aws.guardduty", "secureai.demo"] })
}
resource "aws_cloudwatch_event_target" "lambda" {
  rule           = aws_cloudwatch_event_rule.finding.name
  event_bus_name = aws_cloudwatch_event_bus.this.name
...
resource "aws_lambda_permission" "events" {
  ...
  source_arn    = aws_cloudwatch_event_rule.finding.arn
}
